...
ax.scatter(interesting_pelvis_coor_gt[:, 2], interesting_pelvis_coor_gt[:, 1],c='blue', label='GROUDTRUTH', s=3)
ax.scatter(interesting_pelvis_coor_sample[:, 2], interesting_pelvis_coor_sample[:, 1],c='red', label='SAMPLE', s=3)

plt.axis('equal')
# plt.show()

plt.savefig('../MOCAP_results/PELVIS/pelvis_MOCAP_groundTruthVSsample_sampledTraj_'+str(sample)+'.png')

############### computing ADE

### vline
points_sample = np.array(interesting_pelvis_coor_sample)
points_gt = np.array(interesting_pelvis_coor_gt)
...
z_values = interesting_pelvis_coor_gt[:,2]
points_array = np.column_stack((x_values, y_values, z_values))
# points_array = np.column_stack((z_values, _values, x_values))
# points_array -= points_array[0]

fig_pelvis_traj = plt.figure()
ax = fig_pelvis_traj.add_subplot(111)
ax.scatter(points_array[:,2],points_array[:,1], linestyle="-")
ax.scatter(interesting_pelvis_coor_gt[:, 2], interesting_pelvis_coor_gt[:, 1],c='blue', label='GROUDTRUTH', s=3)
ax.scatter(interesting_pelvis_coor_sample[:, 2], interesting_pelvis_coor_sample[:, 1],c='red', label='SAMPLE', s=3)
plt.axis('equal')
plt.show()
...
# plt.show()
plt.savefig('../MOCAP_results/PELVIS/pelvis_MOCAP_groundTruthVSsample_sampledTraj_Distances_'+str(sample)+'.png')
