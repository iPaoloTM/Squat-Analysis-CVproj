...
def load(filename, start=None, end=None, order=None, world=False):
    ...
    for line in f:
        ...
        if dmatch:
            ...
            print("c-----",channels)
            print("c-----",positions.shape)
            print("c-----",data_block.shape)
            if   channels == 3:
                # This should be root positions[0:1] & all rotations
                print("data_block",data_block.shape)
                positions[fi,0:1] = data_block[0:3]
                rotations[fi, : ] = data_block[6:].reshape(N,3)
                # rotations[fi, : ] = data_block[3:].reshape(N,3)
                # rotations[fi, : ] = data_block[3:66].reshape(N,3)
            elif channels == 6:
                data_block = data_block[3:].reshape(N,6)
